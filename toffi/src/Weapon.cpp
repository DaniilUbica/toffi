...
bool Weapon::isInAttackRange(sf::Vector2f pos, std::shared_ptr<Character> character, float range) {
...
void Weapon::commonUpdate(sf::Vector2f pos, std::vector<std::shared_ptr<Character>>& characters, float attack_range) {
	if (!characters.empty()) {
		...
		if (nearest != characters.end() && *nearest) {
			...
			m_angle = std::atan2(m_direction.y, m_direction.x) * 180 / PI;
