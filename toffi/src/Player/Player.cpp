...
void Player::checkCollisionWithMapBorders() {
    if (m_pos.y >= game_engine::WORLD_HEIGHT - m_size.y) {
        m_pos.y = game_engine::WORLD_HEIGHT - m_size.y;
    }
    if (m_pos.y <= game_engine::SPRITE_SIZE) {
        m_pos.y = game_engine::SPRITE_SIZE;
    }
    if (m_pos.x >= game_engine::WORLD_WIDTH - m_size.x) {
        m_pos.x = game_engine::WORLD_WIDTH - m_size.x;
    }
